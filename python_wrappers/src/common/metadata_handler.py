...
class MetadataHandler(run_info.RunInfo):
    ...
    def _update_attr_from_json(self) -> None:
        ...
        if _tmp == "single_channel":
            self.n_channels = 1
            
            _tmp_single = meta_data.get("channel")
            self.channel = int(_tmp_single)
            
            _tmp_single = meta_data.get("board")
            self.board = int(_tmp_single)
            
            _tmp_single = meta_data.get("threshold")
            self.threshold_adc = int(_tmp_single)
                
        elif _tmp == "all_channels":
            _tmp_all = meta_data.get("channel_list")
            self.n_channels = len(_tmp_all)
            self.channel_list = _tmp_all
        
        else:
            raise ValueError(f"Unknown data taking mode: {_tmp}")
    ...
    def _get_paired_files(self) -> None:
        ...
        if self.data_taking_mode == "single_channel":
            ...
            basename = os.path.basename(self.md_full_path)
            ...
            if match:
                ...
                assert threshold_adc == self.threshold_adc
                ...
                raise ValueError(f"The file format for single channel data files: {self.md_full_path} does not match. \n" + \
            ...
            if match:
                ...
                raise ValueError(f"The file format for all channel data files: {self.md_full_path} does not match. \n" + \
                                f"It should be in 'meta_config_all" + \
                                f"_{{date_YYYYMMDD}}_{{time_HHMMSS}}.json'")
