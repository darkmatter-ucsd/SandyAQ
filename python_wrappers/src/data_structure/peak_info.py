...
class PeakInfo(WaveformInfo):
    ...
    def __init__(self):
        ...
        self.channel_id: int = np.nan
        self.event_id: int = np.nan
        self.waveform_id: int = np.nan
    
        self.spe_position: int = np.nan

        # peak level information
        self.peak_id: int = np.nan
        ...
        self.peak_rel_start_time_s: float = np.nan  # seconds since run start time
    ...
    #         self.peak_height_V_array[i] = np.max(single_processed_waveform[peak_boundary[0]:peak_boundary[1]])
    ...
    #         start_sample = max(peak_boundary[0]-extend_sum_window, 0)
    #         end_sample = min(peak_boundary[1]+extend_sum_window, len(single_processed_waveform))
