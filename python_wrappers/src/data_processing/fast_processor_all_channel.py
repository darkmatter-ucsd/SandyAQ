...
class FastProcessor:
    ...
    def process_runs(self, n_random_WF = 100):
        if(len(self.list_of_files))==1:
            self.list_of_fast_info = self.process_run_single_run(
                self.list_of_files[0], 
                n_random_WF = n_random_WF)
            # otherwise it will loop over the single file path
            # e.g. "/", "h", "o", "m", "e", ...
        else:
            self.list_of_fast_info = self._v_process_run_single_run(
                self, md_full_path = np.array(self.list_of_files), 
                n_random_WF = n_random_WF)
    ...
    def process_run_single_run(self, md_full_path, n_random_WF = 100):
        ...
        if (RunProcessor.failure_flag == True) or (RunProcessor.info.data_taking_mode != "all_channels") or (len(RunProcessor.bin_full_path_list) != 2):
        ...
        for channel_level_data, extra_data in zip(result[0], result[1]):
            ...
            print(f"Channel: {fast_info.channel}")
            print(f"Integral window: {fast_info.integral_window}")
            ...
            tmp = deepcopy(fast_info)
            fast_info_list.append(tmp)
            ...
            #     fast_info.rise_time_ns = RunProcessor.EventProcessor.wfp.get_rise_time_ns(search_window=fast_info.integral_window)
                ...
                # tmp = RunProcessor.EventProcessor.get_randomly_selected_WF(
                #     rel_channel,
                #     n_random_WF,
                #     )
        ...
        if isinstance(self.list_of_fast_info, list) or isinstance(self.list_of_fast_info, np.ndarray):
            ...
            for fast_info in self.list_of_fast_info:
                if len(channels) != 0 and (fast_info.channel in channels):
                    logger.info(f"Skipping channel {fast_info.channel}")
                    continue
                else:
    ...
    def plot_waveform_single_run(self, single_run_info:FastInfo, show_plot = False, save_plot = True):
        ...
        axes[2,0].hist(single_run_info.areas_Vns,
                        # bins=single_run_info.EventProcessor.hist_n_bins,
                        bins=single_run_info.EventProcessor.hist_n_bins,
                        # range=single_run_info.EventProcessor.hist_range,
                        range=(-0.1,10),
                        histtype='step',
                        color='red', 
        ...
        axes[2,1].hist2d(single_run_info.areas_Vns,single_run_info.heights_V,
                         bins=[single_run_info.EventProcessor.hist_n_bins,100],
                         range=[list(single_run_info.EventProcessor.hist_range),[0,0.06]],
                         cmap='viridis',
                         norm=LogNorm())
        ...
        if save_plot:
            ...
            plt.savefig(os.path.join(figure_path,f"plot_{single_run_info.date_time_str}_{single_run_info.channel}_{today}.png"))
