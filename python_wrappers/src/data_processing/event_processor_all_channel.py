...
class EventProcessor:
    def __init__(self, info: RunInfo) -> None:
        ...
        """
        Args:
            event_info (EventInfo): EventInfo object containing all the information needed
        ...
        ### check parameters
        # self.info.number_of_events = int(self.info.number_of_events)
        # self.info.record_length_sample = int(self.info.record_length_sample)
        # if not isinstance(self.info.n_channels, int)
        if not os.path.exists(self.info.bin_full_path):
            raise FileNotFoundError(f"File not found: {self.info.bin_full_path}")
        if not np.issubdtype(type(self.info.number_of_events), int) or self.info.number_of_events <= 0:        
        ...
        if (self.info.number_of_events <= (self.truncate_event_front + self.truncate_event_back)):
            raise ValueError(f"number_of_events should be greater than the set truncate_event_front ({self.truncate_event_front}) and truncate_event_back ({self.truncate_event_back}) values, got {self.info.number_of_events}")
        if not np.issubdtype(type(self.info.record_length_sample), int) or self.info.record_length_sample <= 0:
        ...
        if not np.issubdtype(type(self.info.n_channels), int) or self.info.n_channels <= 0 or self.info.n_channels > 32:
            raise ValueError(f"n_channels should be a positive integer, got {self.info.n_channels}")
        # if not isinstance(self.info.post_trigger, float) or self.info.post_trigger < 0 or self.info.post_trigger > 100:
        if not np.issubdtype(type(self.info.post_trigger), float) or self.info.post_trigger < 0 or self.info.post_trigger > 100:
    ...
    def get_waveform_processor(self, channel: int) -> waveform_processor.WFProcessor:
        """_summary_

        Args:
            channel (int): channel number to get the waveform processor for

        Returns:
            waveform_processor.WFProcessor: waveform processor object
        """
        if (self.info.number_of_events <= (self.truncate_event_front + self.truncate_event_back)):
        ...
        if (type(self.waveform) is not np.ndarray):
            self.failure_flag = True
            logger.warning("Waveform data is not ndarray. " + \
                             "Please check the waveform data reading process.")
            return None
    ...
    def process_all_events(self, channel: int) -> None:
        ...
        wfp = self.get_waveform_processor(channel)
