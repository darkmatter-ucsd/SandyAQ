...
output_fname = "/kalinka/storage/darkmatter/XENONnT/sk6801/UCSD_data/processed_data/peak_all_channel_event_rate.csv"
output_fname = util.get_new_filename(output_fname, use_datetime=True) ## in case the file already exists, create a new name

logger.info(f"Output file: {output_fname}")

# df = pd.DataFrame(columns=["md_full_path", "board", "comment", "run_tag", "voltage_preamp1_V",
#             "data_taking_mode", "n_processed_events", "threshold_adc", "integral_area_Vns_board_channel0",
#             "absolute_time"])
...
basename = os.path.basename(output_fname).split('.')[0]
